...
fall back.** `"./*"` is pure string substitution. `@tev-ui/ui/primitives/Alert`
only resolves because `"./primitives/*"` maps explicitly to
`.../*/index.js`. Once that pattern exists, `@tev-ui/ui/primitives/Alert/index`
...
working spelling (`@tev-ui/ui/ui/primitives/Alert/index`) and turned every typo
...
- Inside `src/`, imports are **always relative**. There is deliberately no
  `@tev-ui/ui/*` path mapping or Vite alias: an alias resolves to source and would
